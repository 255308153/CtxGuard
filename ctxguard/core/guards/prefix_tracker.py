...
_NON_SEMANTIC_KEYS = frozenset(
    {
        ...
        "index",
        # per-message display name that some clients add/drop across turns;
        # replaying the previous forwarded bytes keeps the wire stable anyway
        "name",
    }
)
...
class SessionTrackerStore:
    ...
    def is_primary_lineage(self, session_id: str, tracker: PrefixCacheTracker) -> bool:
        """True if ``tracker`` is the session's first lineage.

        The first lineage rides the bare session id, so its state can be
        persisted/restored via the session_cache table across worker
        processes. Later lineages only exist in memory. Callers use this to
        decide which state source (session-wide dicts vs the tracker itself)
        drives freeze/replay decisions.
        """
        return self._trackers.get(session_id) is tracker
