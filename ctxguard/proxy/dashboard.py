...
<body class="min-h-screen flex flex-col antialiased selection:bg-brand-500 selection:text-white">
  ...
  <main class="flex-1 max-w-7xl w-full mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-8">
    ...
    <section class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-5">
      ...
      <div class="glass p-5 rounded-2xl glow-emerald">
        <div class="flex items-center justify-between">
          <span class="text-xs font-semibold uppercase tracking-wider text-gray-400">累计节省 Token</span>
          <span class="p-2 rounded-xl bg-brand-500/10 text-brand-400">
            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 10V3L4 14h7v7l9-11h-7z"/></svg>
          </span>
        </div>
        <div class="mt-4 flex items-baseline justify-between">
          <span id="stat-saved-tokens" class="text-3xl font-extrabold text-white">0</span>
          <span id="stat-saved-percent" class="text-sm font-bold text-brand-400 bg-brand-500/10 px-2 py-0.5 rounded-md">0.0%</span>
        </div>
        <p class="mt-2 text-xs text-gray-500">优化后上下文开销大幅精简</p>
      </div>

      <!-- Card 2 -->
      <div class="glass p-5 rounded-2xl">
        <div class="flex items-center justify-between">
          <span class="text-xs font-semibold uppercase tracking-wider text-gray-400">预估节省资金</span>
          <span class="p-2 rounded-xl bg-blue-500/10 text-blue-400">
            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8c-1.657 0-3 .895-3 2s1.343 2 3 2 3 .895 3 2-1.343 2-3 2m0-8c1.11 0 2.08.402 2.599 1M12 8V7m0 1v8m0 0v1m0-1c-1.11 0-2.08-.402-2.599-1M21 12a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>
          </span>
        </div>
        <div class="mt-4 flex items-baseline justify-between">
          <span id="stat-dollars-saved" class="text-3xl font-extrabold text-white">$0.0000</span>
          <span class="text-xs font-medium text-gray-400">USD 折算</span>
        </div>
        <p class="mt-2 text-xs text-gray-500">根据模型官方定价实时折算</p>
      </div>
      ...
      <div class="glass p-5 rounded-2xl">
        ...
        <div class="mt-4 flex items-baseline justify-between">
          <span id="stat-total-requests" class="text-3xl font-extrabold text-white">0</span>
          <span class="text-xs font-medium text-purple-400">Reqs</span>
        </div>
        <p class="mt-2 text-xs text-gray-500">已处理的端到端请求总数</p>
      </div>

      <!-- Card 4 -->
      <div class="glass p-5 rounded-2xl">
        <div class="flex items-center justify-between">
          <span class="text-xs font-semibold uppercase tracking-wider text-gray-400">网关处理延迟</span>
          <span class="p-2 rounded-xl bg-amber-500/10 text-amber-400">
            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>
          </span>
        </div>
        <div class="mt-4 flex items-baseline justify-between">
          <span id="stat-avg-latency" class="text-3xl font-extrabold text-white">0.00</span>
          <span class="text-xs font-medium text-amber-400">ms 平均</span>
        </div>
        <p class="mt-2 text-xs text-gray-500">极轻量纯算法，<3ms 透明转发</p>
      </div>
    </section>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div class="glass rounded-2xl p-6">
          <div class="flex items-center justify-between pb-4 border-b border-dark-border">
            <div>
              <h2 class="text-base font-bold text-white flex items-center space-x-2">
                ...
                <span>实时在线压缩测试工作台</span>
              </h2>
              <p class="text-xs text-gray-400 mt-0.5">粘贴任意代码、JSON 或终端日志，实时查看 6 层流水线压缩效果对比</p>
            </div>
          ...
          <div class="mt-4 grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              ...
              <textarea id="test-input" rows="12" class="w-full rounded-xl bg-dark-input border border-dark-border p-3 text-xs font-mono text-gray-200 focus:outline-none focus:border-brand-500 transition resize-none placeholder-gray-600" placeholder="在此粘贴测试文本、JSON 数组或报错日志..."></textarea>
            </div>
            <div>
              <div class="flex justify-between items-center text-xs font-semibold text-gray-400 mb-1.5">
              ...
              <textarea id="test-output" rows="12" readonly class="w-full rounded-xl bg-dark-input/60 border border-dark-border p-3 text-xs font-mono text-emerald-300 focus:outline-none resize-none placeholder-gray-600" placeholder="点击下方按钮运行测试流水线..."></textarea>
            </div>
          </div>

          <div class="mt-5 flex items-center justify-between pt-4 border-t border-dark-border">
            <div id="test-compressors-tags" class="flex flex-wrap gap-1.5 text-xs text-gray-400 items-center">
              <span>激活算子:</span>
              <span class="text-gray-500 italic text-xs">等待运行...</span>
            </div>
        ...
        <div class="glass rounded-2xl p-6">
          <div class="flex items-center justify-between pb-4 border-b border-dark-border">
            <h2 class="text-base font-bold text-white flex items-center space-x-2">
              ...
              <span>最近代理请求流水 (Request Audit Logs)</span>
            </h2>
            <span class="text-xs text-gray-400">实时记录</span>
          </div>

          <div class="mt-4 overflow-x-auto">
            <table class="w-full text-left text-xs">
              <thead class="text-gray-400 font-semibold border-b border-dark-border/60 uppercase">
                <tr>
                  <th class="pb-2.5">ID</th>
                  <th class="pb-2.5">时间</th>
                  <th class="pb-2.5">模型</th>
                  <th class="pb-2.5">原始 Tokens</th>
                  <th class="pb-2.5">优化后</th>
                  <th class="pb-2.5">节省率</th>
                  <th class="pb-2.5">网关耗时</th>
                </tr>
              </thead>
              <tbody id="recent-table-body" class="divide-y divide-dark-border/40 font-mono text-gray-300">
                <tr><td colspan="7" class="py-4 text-center text-gray-500 font-sans">暂无历史请求记录</td></tr>
              </tbody>
            </table>
          </div>
        </div>
      ...
      <!-- Right Column: Quick Setup & Offline Learn (1 Col) -->
      <div class="space-y-6">
        ...
        <div class="glass rounded-2xl p-6">
          <div class="flex items-center justify-between pb-3 border-b border-dark-border">
            <h2 class="text-base font-bold text-white flex items-center space-x-2">
              ...
              <span>离线自学习与规则落盘</span>
            </h2>
          </div>

          <p class="mt-3 text-xs text-gray-400">挖掘交互报错与死循环，将因果反思幂等更新到本地规则文件。</p>
          ...
          <div class="mt-4 space-y-3">
            ...
            <div id="learn-result" class="hidden p-3 rounded-xl bg-dark-input font-mono text-[11px] text-gray-300 border border-dark-border max-h-48 overflow-y-auto"></div>
          </div>
        </div>
  ...
  <footer class="border-t border-dark-border/40 py-6 mt-12 text-center text-xs text-gray-500">
    <p>CtxGuard Context Optimization Gateway • 零重型依赖 • 毫秒级极速优化 • 开源自由</p>
  </footer>
        ...
        if (recent.length > 0) {
          tbody.innerHTML = recent.map(r => `
            <tr class="hover:bg-dark-card/40 transition">
              <td class="py-2.5 text-gray-500">#${r.id}</td>
              <td class="py-2.5 text-gray-400">${(r.timestamp || '').slice(11, 19)}</td>
              <td class="py-2.5 text-blue-400">${r.model}</td>
    ...
    async function runTestCompress() {
      ...
      try {
        ...
        if (compressors.length > 0) {
          tagsContainer.innerHTML = '<span>激活算子:</span>' + compressors.map(c => `
            <span class="px-2 py-0.5 rounded bg-brand-500/20 text-brand-400 text-[11px] font-mono border border-brand-500/30">${c}</span>
          `).join('');
        } else {
          tagsContainer.innerHTML = '<span>激活算子:</span><span class="text-gray-500 italic text-xs">无修改 (内容已最优)</span>';
        }
      } catch (err) {
